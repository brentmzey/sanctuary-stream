...
impl SanctuaryBridge {
                        ...
                        if ev.event_type == "commands" {
                            if let Ok(data) = serde_json::from_str::<serde_json::Value>(&ev.data) {
                                if data["action"] == "create" {
                                    if let Ok(record) = serde_json::from_value::<CommandRecord>(data["record"].clone()) {
                                        if !record.executed {
    ...
    async fn execute_command(
        ...
        let result: Result<(), Box<dyn std::error::Error + Send + Sync>> = match command.action.as_str() {
            "START" => obs.streaming().start().await.map(|_| ()).map_err(|e| e.into()),
            "STOP" => obs.streaming().stop().await.map(|_| ()).map_err(|e| e.into()),
            "RECORD_START" => obs.recording().start().await.map(|_| ()).map_err(|e| e.into()),
            "RECORD_STOP" => {
            ...
            _ => {
                warn!("Unknown command action: {}", command.action);
                Ok(())
            }
        };
